...
- 普通聊天和工作群组共享 `ConversationDetailScaffold` / `ConversationPaneScaffold` 几何骨架，并复用 `ConversationHeader`、`ConversationHeaderActionButton` 与 `ConversationComposer*` 组件统一标题区、右侧图标操作、KB/快捷区、输入框和发送/停止按钮。普通聊天与 Workflow Team Channel 还共用 `ConversationRoomHeaderActions`，固定提供会话文件、Markdown/Obsidian 导出、邀请和单一成员入口（支持 Vault 时多一个目录选择按钮）；`ConversationRoomMembersDialog` 统一成员列表和面板内添加成员，聊天群组以联系人作为候选，Workflow Owner 通过用户搜索添加并可移除成员。两者还通过 `ConversationRoomComposerTools` 共用目录上传、文件上传、截屏和语音输入；截图预览及发送也走同一 Room 级实现。Workspace Agent 仍只保留编码会话控制，不展示这些依赖具体 Room 文件空间的工具；聊天多选与 Workspace Agent/权限/代码审查仍由各自主体提供。固定区使用 `silk-conversation-fixed-region`，消息区是唯一的 `silk-conversation-scroll-region`，业务状态和消息 scope 仍由各自主体管理
