...
- `kb/KnowledgeBaseManager.kt`:
  ...
  - `searchEntriesForContext(...)`：按用户可见性 + preferredGroup 检索已发布条目，供上下文自动补充
- `kb/KnowledgeBasePromptContext.kt`:
  - `resolveKnowledgeBasePromptContext(...)`：解析聊天消息中的 `[[kb:...]]` 内联引用（手动），叠加用户固定(`pinnedEntryIds`)、自动检索候选，剔除排除项(`excludedEntryIds`)，注入匹配条目内容作为 AI prompt 上下文，并返回 `diagnostics`（手动/固定/自动/排除计数）；取代旧 `KnowledgeBaseReferenceResolver`
...
- KB HTTP 在 `Routing.kt` 的 `/api/kb/*`（含 `PUT /api/kb/topics/{id}` 改主题/访问策略、`POST /api/kb/captures` 入库候选；既有路由按调用方 userId 做读写/成员可见性鉴权）
