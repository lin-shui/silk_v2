...
bash scripts/deploy.sh                 # 完整流程: 安装依赖 → 构建 → 部署 → 启动
bash scripts/deploy.sh --skip-install  # 跳过 ohpm install（依赖没变时）
bash scripts/deploy.sh --log           # 部署后自动拉取日志
```
...
```bash
DEVECO_HOME="/your/path/to/DevEco Studio" bash scripts/deploy.sh
```
...
> 注意: Windows 上需要 Java 在 PATH 中（`$DEVECO_HOME/jbr/bin`），deploy.sh 已自动处理。
