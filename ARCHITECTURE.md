...
**回复图片**：cc-connect Agent 生成图片（SVG/PNG 等，通过 Write/Bash 等工具）后，`silks.go` 的 `Finalize` 会检测工作目录中本次 turn 内新增/修改的图片文件，base64 编码后通过 `reply_images` 消息发送给 Silk。Silk 后端将其保存到 `chat_history/` 并以 `##PREVIEW_IMAGE:` 标记广播，前端自动以内联图片渲染。限制：仅扫描项目根目录（非递归），单次最多 5 张，单张 ≤10MB。
