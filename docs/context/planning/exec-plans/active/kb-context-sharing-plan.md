...
- 当前剩余缺口：
  - **KB Copilot 流式响应与内联 Diff 编辑**（计划已写入 `exec-plans/active/2026-07-10-kb-copilot-streaming-inline-edit.md`）：当前同步 HTTP 体验差，模型推理时用户空转等待；生成的草稿"填回编辑器"是全量替换而非 VS Code Copilot 式的内联 diff + accept/reject
  - 更完整的 candidate merge 策略，尤其是冲突 diff / review
  - Harmony 端 bearer caller 对齐
  - 空间级降权等更细的召回策略
  - ~~群会话文件和知识库浏览已打通 MVP，但 KB 对音频 / 视频等非 Markdown 资产还缺原生预览面，难以承接群文件沉淀~~（2026-07-10 已补：后端下载接口扩展为图片/PDF/音频/视频均以 Inline 响应；Web 端新增 `KnowledgeFilePreview` 组件族 —— 图片灯箱、PDF embed、音频/视频播放器 —— 集成到 `GroupFilesContent` 的"预览"按钮和 `KnowledgeEditorPane` 的 inline 文件预览区；Android 端 `KnowledgeBaseEditorPage` 新增 `KnowledgeFilePreviewCard` 使用 WebView 预览图片/PDF/音视频）
