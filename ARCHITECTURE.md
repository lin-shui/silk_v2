...
Workspace 创建（含 GitHub Issue 入口）从所有者已登记的具体 Agent 实例中选择，目录浏览、信任、`/cd` 和初始 ACTIVE Workspace Binding 都固定到同一 `agentInstanceId`；旧 type-only 请求继续兼容。设备与 Agent 所有者可修改显示名，重命名不改变设备身份、Agent 类型、连接或授权关系。
...
5. Claude Code / Codex 的 ACP adapter 由 `silk-agent` 托管，通过本地 Host IPC v2 接入单一 `/agent-connect` 设备 WSS；兼容 `/agent-bridge` 只接受设备签名旧 Host。外部 `cc_bridge/acp_adapter.py` 和 `codex_bridge/codex_adapter.py` 跑各自 CLI 并流式回传。
   ...
   - `/agent-connect` 已在一次设备签名认证后承载版本化 `agent_open / agent_rpc / agent_close` envelope，并按 `agentInstanceId` 多路复用 Claude Code 与 Codex ACP 逻辑流；每个流独立做 Agent 状态、能力、Binding 和权限检查。新 Host 的 `agent_open` 还会用设备私钥签名当前 connector 版本和能力，后端只把白名单中的 `EXECUTION_POLICY_V2` 自动合并到原 AgentInstance，保留设备、Binding、审批和会话；旧 Host 不发送刷新元数据时继续 fail-closed。`/agent-bridge` 仅保留旧 Host 设备签名兼容握手，query Token 明确拒绝，`/ccconnect-bridge` 仍使用旧群组 Token。
   ...
   - Web `/device` 页面复用现有 JWT 登录，区分新设备/新增 Agent 的配对预览与批准/拒绝，并提供设备与 Agent 在线状态/撤销、Agent 运行权限及 Room/Workspace Binding 增删改、Workspace 访问模式和双主体审批；Room/Team Channel 与 Workspace 头部也提供目标上下文内的 Agent 管理入口。用户可配置的权限只保留两层：Agent owner 在 `/device` 维护的 `NATIVE_DEFAULT / APPROVAL_REQUIRED / READ_ONLY / AUTOMATIC` 运行覆盖，以及 Silk Workspace Binding 的 `READ_ONLY / APPROVAL_REQUIRED / AUTONOMOUS`；Room Binding 内部固定为 `CHAT_ONLY`。Agent 覆盖保存于 Silk SQLite，不改设备上的 Claude/Codex 配置，也不要求 Host 重启；`NATIVE_DEFAULT` 才委托给设备原生配置，其余选择从下一轮 prompt 开始生效。服务端按两层中更严格的一层计算有效策略，Workspace 审批/只读不会被 Agent 自动执行放宽。跨所有者 Binding 可由 Agent owner 或目标管理者发起，只有 Agent owner 与 Room HOST/OPERATOR（或 Workspace owner）均批准后才进入 ACTIVE；身份、能力、双主体审批、绑定目录和 sandbox 是不可绕过的安全合同，不是额外的用户权限档位。Room Binding 拥有 Room 内唯一的 mention alias，因此同一类型的多个 AgentInstance 可分别用 `@cc`、`@cc2` 等名称路由。设备签名 Agent 按精确 `agentInstanceId` 检查 ACTIVE Binding、内部权限投影与声明能力；新 Host 在已认证的 `agent_open` 中签名当前版本/能力，后端仅自动加入 `EXECUTION_POLICY_V2`，不改变 AgentInstance、设备、Binding、审批或会话；旧 Host 未发送刷新元数据时继续 fail-closed。每次受管 prompt 还要求 `EXECUTION_POLICY_V2` 并携带含有效 `accessMode`、`agentPermissionMode` 的 `_silk.executionPolicy`。`READ_ONLY` 禁止本地写入和命令，并让 Codex app-server 使用 `approvalPolicy=never`；`APPROVAL_REQUIRED` 让 Claude 使用 `manual`，Codex 使用 app-server 的 `on-request + read-only sandbox`，两边都能在执行前生成统一 Silk 权限卡片；仅当 Agent 与 Workspace 两层都允许时，`AUTOMATIC` 才让 Claude 使用 `bypassPermissions`、Codex 使用 `approvalPolicy=never`。Silk 始终保留路径检查、deny、sandbox 与 shell gate，且不会启用 Codex dangerous no-sandbox bypass。缺少 V2 能力的旧登记 Agent 会拒绝 prompt，升级当前 Host 后自动刷新，无需重新登记。Ktor 的同名固定路由会在 Nginx fallback 场景返回已构建 SPA `index.html`。
...
8. `WorkspaceManager` 在 `workspace_store.json` 持久化工作目录、active agent 类型与精确 `agentInstanceId`、per-instance session、可见性、最后共享名称、Co-pilot 和 `ACTIVE/ARCHIVED` 生命周期；Workspace 访问模式只存于 SQLite `agent_bindings.access_mode`，不再在 runtime/Workspace 文件中维护另一份权限状态。旧记录没有实例 ID 时继续使用唯一 ACTIVE Binding 的类型级兼容解析，`workflow_store.json` 暂保留工作流入口元数据并作为一次性迁移源。Workflow Room 连接只恢复已显式创建且 cwd 非空的活动工作区，不会隐式创建默认编码工作区。SHARED 转 PRIVATE 后，Observer 的历史入口只返回 Owner 与最后共享名称，不暴露私有阶段名称或 runtime 元数据。
9. Web 端以单一“会话”入口消费 `GET /api/rooms/visible`，Silk 专属会话固定置顶，其余普通聊天和工作群组按最近活动倒排：有消息时使用最后消息时间，无消息时使用创建时间。当前 Room 的持久化消息通过已有 WebSocket 在本地立即更新排序，15 秒 Room 列表轮询仅作为其他 Room、跨设备活动和未读状态的兜底；侧栏 `+` 统一承载创建群组和通过邀请码加入，Room 行菜单按 JWT 角色提供邀请、重命名、退出和删除，操作成功后本地立即更新列表。联系人位于主导航底部、设置齿轮上方，退出登录位于设置页账户区。普通聊天与 Workflow Team Channel 共用 `@Silk` 快捷按钮并触发同一 `TEAM` Silk AI 链路，Workspace 则继续使用独立的编码 Agent 链路；Room/Team Channel 和 Workspace 头部可直接添加已登记 Agent，选择项显示 Agent 类型、设备名与在线状态，`/device` 保留全局库存和安全管理职责。Room 详情共同复用 header、图标操作按钮、上下文/快捷区、输入行和发送/停止按钮几何，普通聊天与 Team Channel 还共用 Room 级会话文件、Markdown/Obsidian 导出、邀请、单一成员入口和成员面板，以及目录上传、文件上传、截屏和语音输入工具；添加成员收敛到成员面板内部，两类 Room 均先展示未入群联系人并可搜索其他用户，成员搜索允许单字符输入并按精确度进行模糊排序，管理权限仍由 Room 类型决定。Workspace Agent 不展示依赖 Room 文件空间的四项工具，聊天多选与 Workspace Agent 控制仍由各自主体提供。桌面 Room 侧栏和工作群组树可折叠并持久化偏好，`<= 1100px` 的列表/详情单页布局会忽略侧栏偏好并保持列表完整可用。
...
- 外部 Agent 身份：SQLite `agent_devices`、`agent_instances`、`agent_bindings`、`agent_binding_audit_events`、`agent_pairing_requests`、`agent_device_revocation_tombstones`；`agent_instances.runtime_permission_mode` 保存 owner 选择的 Agent 运行覆盖，变更写入安全事件，设备原生 CLI 配置不进入 Silk。Binding 的安全相关状态变化写入不可变快照审计，设备表只保存 Ed25519 公钥，配对短码和轮询 secret 只保存 SHA-256 摘要。撤销设备/Agent 的业务行默认保留 90 天（`SILK_AGENT_REVOKED_RETENTION_DAYS` 可调），后台每日自动清理到期行；安全事件、Binding 审计和已撤销设备公钥 tombstone 保留。
...
- Personal Workspace 运行状态：`~/.silk-data/workflows/workspace_store.json`（同时保存按 `ownerId + roomId + agentInstanceId` 隔离的最近成功工作目录，用于后续新建 Workspace 的目录默认值；二者均可用 `SILK_WORKFLOW_DIR` 或 `-Dsilk.workflowDir=...` 覆盖）
