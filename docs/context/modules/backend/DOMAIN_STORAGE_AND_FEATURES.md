...
- `kb/KnowledgeBaseManager.kt`:
  ...
  - 访问控制贯穿所有 KB 路由：`canReadTopic/canWriteTopic/canManageTopic`、成员可见性、`writeLocked`；调用方身份优先经 `Authorization: Bearer <app_auth_token>` 解析（`UserSettingsRepository.findUserIdByAppAuthToken`），无 Bearer 时回退到 `X-Silk-Authenticated-User-Id` 头
