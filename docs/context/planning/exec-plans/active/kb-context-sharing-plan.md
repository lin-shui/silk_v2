...
- `2026-07-06`: 已继续补强 7.1 的窄宽度可用性：KB 编辑器的“编辑 / 预览 / 分栏”模式切换不再只靠单字标签硬撑，现按可用宽度在完整 segmented control、紧凑单字按钮、下拉选择三档之间切换；同时补了逻辑单测，避免后续回退到三项被挤没的状态。
- `2026-07-06`: 已补齐 7.1 菜单态的最后一段桌面交互收口：KB 编辑器右上角“菜单”展开后支持点击外部区域关闭与 `Esc` 关闭，避免低频动作收纳后出现悬浮菜单滞留。
