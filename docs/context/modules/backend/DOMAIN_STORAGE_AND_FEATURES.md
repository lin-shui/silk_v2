...
- `ChatHistoryManager.kt`:
  ...
  - 成员列表
  - uploads 目录协同
  - 持久化消息级 `kbContextSelection`，保留用户对下一轮 KB context 的固定 / 排除选择
...
- `kb/KnowledgeBaseManager.kt`:
  ...
  - Topic / Entry CRUD
  - `POST /api/kb/captures` 把聊天/工作流内容沉淀为 `CANDIDATE` entry，并记录 `sourceType/sourceGroupId/workflowId/messageIds`
  - capture route 支持显式 `status`，但 `CHAT` / `WORKFLOW` 来源会被后端强制回落到 `CANDIDATE`；`MEETING` / `FILE` / `URL` 等来源可按调用方传入 `status` 建条，供未来会议纪要或导入流复用统一入口
  ...
  - `resolveKnowledgeBasePromptContext()` 会同时处理手动 `[[kb:...]]`、消息里传入的 pinned/excluded 选择，以及自动 lexical 候选，顺序为 手动 > 固定 > 自动
