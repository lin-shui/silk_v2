...
- `workflow/WorkflowManager.kt`:
  ...
  - `sessionId` 字段存的是 Claude CLI 的真实 session id（即 ACP 层的 `ccSessionId`），重启后通过 `session/new` 的 `ccSessionId` 参数让 adapter 续会话
  - `updateWorkingDir(groupId, workingDir)` / `updateSessionState(groupId, sessionId, sessionStarted)` 由 `AgentRuntime.WorkflowPersistence` 回调驱动；写入跳过等值无变化以省 I/O
- `kb/KnowledgeBaseManager.kt`:
  - `knowledge_base/kb_store.json`
  - Topic / Entry CRUD
- `trust/TrustedDirManager.kt`:
  - `workflows/trusted_dirs.json`
