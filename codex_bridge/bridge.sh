...
BRIDGE_SCRIPT="codex_adapter.py"
...
do_start() {
    ...
    echo -e "${BLUE}启动 Codex Bridge...${NC}"
...
do_stop() {
    ...
    # 发送 SIGTERM，让 codex_adapter.py 优雅关闭
    kill "$pid" 2>/dev/null
...
do_status() {
    if _is_running; then
        local pid
        pid=$(_get_pid)
        echo -e "${GREEN}Codex Bridge 运行中 (PID: $pid)${NC}"
...
case "${1:-}" in
    ...
    *)
        echo ""
        echo -e "${BLUE}Codex Bridge 管理脚本${NC}"
        echo ""
        echo "用法: $0 <命令>"
        echo ""
        echo "命令:"
        ...
        echo ""
        echo "环境变量:"
        echo "  CODEX_AUTO_APPROVE=1  默认带 --dangerously-bypass-approvals-and-sandbox（设为 0 关闭）"
        echo "  CODEX_AGENT_TYPE=codex  默认 agentType（一般不改）"
        echo ""
        ;;
esac
