...
class DirectModelAgent(
     ...
     */
    fun loadRecentHistory(entries: List<com.silk.backend.models.ChatHistoryEntry>, agentIds: Set<String>) {
        if (conversationHistory.isNotEmpty()) return
        val recent = entries.filter { it.messageType == "TEXT" }.takeLast(20)
        if (recent.isEmpty()) return

        for (entry in recent) {
            val role = if (entry.senderId in agentIds) "assistant" else "user"
...
注意：部分平台（标记为 Browser 模式）需要 Chrome 浏览器登录后才能使用。""",
    ...
    private fun getGroupStats(): String {
        return try {
            // 过滤掉 AI 自己的消息
            val userMessages = groupChatHistory.filter { it.senderId != SilkAgent.AGENT_ID && !AgentRuntime.isAgentUserId(it.senderId) }
