...
- 当前剩余缺口：
  - 更完整的 candidate merge 策略，尤其是冲突 diff / review
  - Harmony 端 bearer caller 对齐
  - Harmony 端 bearer caller 对齐
  - 空间级降权等更细的召回策略
  - 群会话文件和知识库浏览仍是两套入口，缺少统一的“群空间资产浏览”与互跳
  - KB 对音频 / 视频等非 Markdown 资产还缺原生预览面，难以承接群文件沉淀
...
| Web 体验 | 三栏 Topic/Entry/Markdown 编辑器；支持 split/preview、复制引用、点击 KB 引用打开文档、Obsidian 导出；已接 personal/team 空间切换、topic 权限 badge、topic/entry 搜索过滤、条目状态/来源 badge、只读态、topic 权限编辑面板、聊天/工作流消息“📚入库”到 candidate、candidate inbox 过滤与批量发布/归档/并入已有文档（已支持同一 knowledge space 的跨 topic merge）、KB 内“会议入库”对话框、去噪后的条目 meta bar provenance 明细、`KB Copilot` 对话框，以及 entry 拖到 topic 的最小拖拽整理交互；Context Tray 在刷新/重连后可恢复最近一条选择并回填空间级持久偏好。 | 仍缺用户选择式分享体验，以及更完整的 merge/review 流。 |
...
| 群文件 / 资产联动 | 聊天/Workflow 已能把消息或摘要“📚入库”为候选；KB 条目 provenance 已能记录 `sourceGroupId` / `workflowId` / `messageIds` 并在 Web 端回跳源消息或工作流。 | 群文件列表与 KB 仍未打通，用户无法在 KB 中浏览群文件，也无法在群会话文件区直接看到群空间已沉淀的知识资产。 |
| 文件预览 | KB 当前主体验仍以 Markdown 文档为中心，文件类更多是“提取文本后入库”。 | 还缺统一的富预览层，尤其是图片、音频、视频、PDF 的原位预览，难以把群文件当作知识空间的一等资产。 |
...
| Group Assets Browser 群资产浏览 | 在 KB 可浏览当前群空间文档/文件，在群会话文件区也能看到已沉淀到知识空间的内容，并支持双向跳转。 | 群文件和知识库本质上都属于同一组协作资产，不应分裂成两套入口。 |
...
| M7 KB Copilot 与群资产打通 | 让 KB 成为“AI 编辑 + 群空间资产浏览”的一等入口。 | KB 页面内 AI Copilot；群文件区与 KB 双向浏览/跳转；文件型资产统一元数据模型；图片/PDF/音视频预览；workflow 级默认知识空间/主题绑定。 | 用户无需切到 Silk 主聊天也能自然语言改 KB；群文件和 KB 可以互相浏览；workflow 中无论内建还是外部 agent，受权限约束的 KB 修改行为一致。 |
