...
- `kb/KnowledgeBaseManager.kt`:
  ...
  - `/api/kb/*` 当前优先从 `Authorization: Bearer <token>` 解析 caller，再兼容旧 `X-Silk-Authenticated-User-Id`；若与 query/body 里的 `userId` 不一致则拒绝请求，未带认证头的旧客户端仍回退到 `userId`
  - `PUT /api/kb/entries/{entryId}` 可更新 `topicId/title/content/tags/status`，供 candidate inbox 做发布/归档流转，也支持把条目移动到同一 knowledge space 内的其他 topic；personal 与 team、或不同 team group 之间的跨空间 move 会被拒绝
