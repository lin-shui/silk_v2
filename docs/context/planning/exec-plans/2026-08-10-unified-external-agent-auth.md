...
- SQLite 已创建 `agent_devices`、`agent_instances`、`agent_bindings`、`agent_binding_audit_events`、`agent_pairing_requests`、`agent_device_revocation_tombstones` 六张表；`DatabaseFactory` 和测试隔离数据库都会创建/补齐这些表。撤销业务行默认保留 90 天后清理，安全事件、Binding 审计和设备公钥 tombstone 保留。
...
- 当前每个设备的每种 Agent 类型仍只允许一个 `ACTIVE` 实例，因此一个 Host 可并存 Claude Code 与 Codex；同一设备内的重复登记仍返回 `AGENT_ALREADY_ENROLLED`。同一账号在不同设备上运行同类型 Agent 不受此设备级约束影响。
- 后端 ACP 运行时索引已改为以 `agentInstanceId` 为主键，并以 user/type 维护只读反向索引；同一账号在多个设备登记同类型 Agent 时连接并存，断线、会话、流式消息、工具状态和撤销均按实例隔离。旧的 type-only helper 仅为未迁移 Workspace/测试调用保留，遇到多个候选时不再猜测。Room Binding 使用唯一 `mentionAlias` 直接路由到精确 AgentInstance；Workspace 若同时绑定多个同类型实例，仍需后续在工作区选择面增加显式实例选择。
