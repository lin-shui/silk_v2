...
- Context Tray 支持“固定下轮 / 排除下轮”最小控制闭环：Web 会把用户选择写进消息合同里的 `kbContextSelection(pinnedEntryIds, excludedEntryIds)`，后端按该选择重建下一轮 KB context；Workflow 面板会把这类 KB context 状态从普通灰色状态列表中过滤掉，避免和 Tray 重复展示
