...
5. Claude Code / Codex 的 ACP adapter 由 `silk-agent` 托管，通过本地 Host IPC v2 接入单一 `/agent-connect` 设备 WSS；兼容 `/agent-bridge` 只接受设备签名旧 Host。外部 `cc_bridge/acp_adapter.py` 和 `codex_bridge/codex_adapter.py` 跑各自 CLI 并流式回传。
   ...
   - Web `/device` 页面复用现有 JWT 登录，区分新设备/新增 Agent 的配对预览与批准/拒绝，并提供设备与 Agent 在线状态/撤销及 Room/Workspace Binding 增删改、细粒度权限选择和双主体审批；Room/Team Channel 与 Workspace 头部也提供目标上下文内的 Agent 管理入口。跨所有者 Binding 可由 Agent owner 或目标管理者发起，只有 Agent owner 与 Room HOST/OPERATOR（或 Workspace owner）均批准后才进入 ACTIVE；Room Binding 拥有 Room 内唯一的 mention alias，因此同一类型的多个 AgentInstance 可分别用 `@cc`、`@cc2` 等名称路由。`AcpRegistry`、Agent session 和流式消息状态均以 `agentInstanceId` 为主键，同一账号不同设备上的同类 Agent 可同时在线且不会互相顶替。设备签名 Agent 的 Room TEAM、Workspace prompt、带 Workspace 目标的目录读取/切换和 Source Control RPC 均按当前连接的精确 `agentInstanceId` 检查 ACTIVE Binding、权限与声明能力。每次受管 prompt 还要求 `EXECUTION_POLICY_V1` 能力并携带版本化 `_silk.executionPolicy`。Claude Code 与 Codex 继承设备用户的原生认证、模型、hooks、MCP、插件和规则；Silk 只对核心本地文件/命令工具增加 Binding 权限上限，Claude 使用显式 deny、工作区路径检查和完整权限下的原生 Bash sandbox，Codex 使用 `read-only/workspace-write` sandbox 与 shell gate，受管路径不启用 dangerous bypass。缺少版本能力的旧登记 Agent 会拒绝 prompt 并需升级后重新登记。Ktor 的同名固定路由会在 Nginx fallback 场景返回已构建 SPA `index.html`。
...
- 外部 Agent 身份：SQLite `agent_devices`、`agent_instances`、`agent_bindings`、`agent_binding_audit_events`、`agent_pairing_requests`、`agent_device_revocation_tombstones`；Binding 的安全相关状态变化写入不可变快照审计，设备表只保存 Ed25519 公钥，配对短码和轮询 secret 只保存 SHA-256 摘要。撤销设备/Agent 的业务行默认保留 90 天（`SILK_AGENT_REVOKED_RETENTION_DAYS` 可调），后台每日自动清理到期行；安全事件、Binding 审计和已撤销设备公钥 tombstone 保留。
